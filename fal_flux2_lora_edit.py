...
class FalAPI_Flux2LoraEdit:
    @classmethod
    def INPUT_TYPES(cls):
        """
        An image generation node for `fal-ai/flux-2/lora/edit`.
        """
        return {
            "required": {
                "api_key": ("STRING", {
            ...
            "optional": {
                ...
                "enable_prompt_expansion": ("BOOLEAN", {
                    "default": False,
                    "display": "Enable Prompt Expansion"
                }),
    ...
    def __init__(self):
        ...
        self.output_dir = os.path.join("output", "API", "FAL", "flux-2-lora-edit")
        ...
        try:
            if isinstance(update, fal_client.InProgress):
                for log in update.logs:
                    print(f"[Flux2LoraEdit] {log['message']}")
        except Exception as e:
            print(f"[Flux2LoraEdit] Error in queue update: {e}")
    ...
    async def generate_image(self, api_key, prompt, image1, width, height,
                             image2=None, image3=None, num_inference_steps=28, guidance_scale=2.5,
                             lora_url="", lora_scale=1.0, seed=-1, output_format="png", 
                             enable_safety_checker=True, enable_prompt_expansion=False, acceleration="regular"):
        
        print("[Flux2LoraEdit] Starting image generation process...")
        ...
        try:
            ...
            print(f"[Flux2LoraEdit] Arguments: {json.dumps(arguments, indent=2)}")
            
            endpoint = "fal-ai/flux-2/lora/edit"
            print(f"[Flux2LoraEdit] Submitting to {endpoint}...")
            ...
            print(f"[Flux2LoraEdit] Downloaded {len(output_images)} images.")
            ...
            print(f"[Flux2LoraEdit] Generation completed in {generation_time} seconds")
            return (batch_tensor, ";".join(saved_paths), json.dumps(result, indent=2), generation_time)

        except Exception as e:
            print(f"[Flux2LoraEdit] Error: {str(e)}")
            # Clean up
            for f in temp_files:
                try: os.remove(f)
                except: pass
            ...
            error_info = {
                "error": f"Flux2LoraEdit generation failed: {str(e)}",
