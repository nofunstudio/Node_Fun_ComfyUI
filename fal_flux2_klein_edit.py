...
class FalAPI_Flux2KleinEdit:
    @classmethod
    def INPUT_TYPES(cls):
        """
        An image generation node for `fal-ai/flux-2/klein/{4b,9b}/base/edit/lora`.
        """
        return {
            "required": {
                "api_key": ("STRING", {
                ...
                "prompt": ("STRING", {
                    "multiline": True,
                    "default": "Make this donut realistic",
                    "display": "Prompt"
                }),
            ...
            "optional": {
                ...
                "negative_prompt": ("STRING", {
                    "multiline": True,
                    "default": "",
                    "display": "Negative Prompt (optional)"
                }),
    ...
    def __init__(self):
        ...
        self.output_dir = os.path.join("output", "API", "FAL", "flux-2-klein-edit")
        ...
        try:
            if isinstance(update, fal_client.InProgress):
                for log in update.logs:
                    print(f"[Flux2KleinEdit] {log['message']}")
        except Exception as e:
            print(f"[Flux2KleinEdit] Error in queue update: {e}")
    ...
    def is_empty_image(self, tensor):
        """Check if image is essentially empty (all black or very dark)."""
        if tensor is None:
            return True
        mean_val = tensor.mean().item()
        return mean_val < 0.01
    ...
    async def generate_image(self, api_key, model_variant, prompt, image1, width, height,
                             image2=None, image3=None, negative_prompt="", num_inference_steps=28, 
                             guidance_scale=5.0, lora_url="", lora_scale=1.0, seed=-1, 
                             output_format="png", enable_safety_checker=True, acceleration="none"):
        
        print(f"[Flux2KleinEdit] Starting image generation process with {model_variant} model...")
        ...
        try:
            ...
            print(f"[Flux2KleinEdit] Arguments: {json.dumps(arguments, indent=2)}")
            
            # Select endpoint based on model variant
            endpoint = f"fal-ai/flux-2/klein/{model_variant}/base/edit/lora"
            print(f"[Flux2KleinEdit] Submitting to {endpoint}...")
            ...
            print(f"[Flux2KleinEdit] Downloaded {len(output_images)} images.")
            ...
            print(f"[Flux2KleinEdit] Generation completed in {generation_time} seconds")
            return (batch_tensor, ";".join(saved_paths), json.dumps(result, indent=2), generation_time)

        except Exception as e:
            print(f"[Flux2KleinEdit] Error: {str(e)}")
            # Clean up
            for f in temp_files:
                try: os.remove(f)
                except: pass
            ...
            error_info = {
                "error": f"Flux2KleinEdit generation failed: {str(e)}",
